...
def get_palette(num_cls):
    ...
    label_colours = [
        ...
        (51, 170, 221),       # Glove
        (0, 0, 255),      # Face v
        (0, 255, 255),   # Left-arm v
        (85, 255, 170),    # Right-arm
        (0, 85, 0),   # Left-leg
        (170, 255, 85),   # Right-leg
        (255, 255, 0),    # Left-shoe
        (255, 170, 0)     # Right-shoe
    ]
    selected_colors = label_colours[:num_cls]

    # Flatten the list of tuples into a single list of integers
    palette = [component for color in selected_colors for component in color]

    # Ensure the palette length is a multiple of 3 by padding with zeros
    if len(palette) % 3 != 0:
        palette.extend([0] * (3 - len(palette) % 3))
